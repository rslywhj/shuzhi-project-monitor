...
- 计划/实际/预测完成月双口径标记、未完成节点计划完成日期、当月指标联动、逾期欠项带入和月度节点下钻
- 项目 × 标准节点红黄绿热力矩阵
...
- 未开始、进行中、暂停、已完成四态节点执行管理，包含实际开始日、暂停原因和状态一致性校验
- 当前主节点、并行节点、前序遗留、应启动未启动及下一节点的可解释阶段识别
- 多节点周度进度填报、项目经理主节点确认与申报/计算进度校验
- 项目阶段随周度快照固化，并贯通项目列表、节点矩阵、时间轴、PDF及PMO数据质量检查
- 周报附件上传、下载、删除及锁定后防篡改
- 红黄节点偏差原因和纠偏措施闭环
- 基线变更审批及版本追溯
- PMO 数据质量检查与周度快照锁定
- D1 数据库与 Workers KV 文件持久化、服务端写入校验和操作审计
- 独立账号密码登录/退出、个人修改密码、管理员重置密码、可配置密码复杂度策略及管理层、PMO、项目经理权限边界
- 已锁定周期防篡改和重复审批保护
- 项目创建、标准节点自动初始化与项目主数据维护 API
- 手工新建项目按独立起止日期自动编排节点计划，支持逐节点校准并冻结原始基线
- Excel 三工作表模板下载、逐行预检和最多100项目的事务化批量导入
- 《统建系统建设及数据治理三级进度计划》原生格式同步：第一个Sheet的36节点基线及明细进度、第二个Sheet的项目清单
- 三级进度表支持后续重复上传更新，对已有项目按系统名称匹配；PMO或管理员确认后可用Excel计划覆盖当前批准基线，并自动生成可追溯的新基线版本
...
- 36节点标准模板持久化，保留阶段、核心工作、交付物、前置条件和风险点，支持权重/关键节点治理与动态大屏矩阵
...
- 企业微信、钉钉、电子邮件和通用 Webhook 外部通知渠道，支持事件订阅、启停和测试
- 外部投递按渠道/项目/周期去重，失败后按5分钟、30分钟、2小时退避重试
- 投递日志、失败原因与人工重试留痕，站内通知始终作为可靠兜底
- Webhook凭据和邮件收件人仅在服务端使用，页面/API全程脱敏；邮件通过 Cloudflare Email Routing 投递
- 基线变更批准/驳回结果自动通知申请人
- 预警规则历史版本查询及当前生效版本标识
...
- 项目存在进行中或暂停节点时，正式周报必须确认一个当前主节点；没有有效确认时系统仅作可解释推荐。
...
清空本地历史数据并重新建立数据库（执行前先停止本地开发服务）：

```bash
npm run db:local:reset
```

重新启动时，`.dev.vars`中`SEED_DEMO_DATA=true`会生成演示数据，设为`false`则保持空项目库。正式环境默认不提供一键硬删除；日常退出监控请使用“结项/归档”，避免破坏快照、审计和基线追溯链。

生产环境需设置 `APP_ADMIN_EMAILS` 和至少 32 位的 `APP_SESSION_SECRET`。管理员通过“系统管理”创建独立账号、初始密码并分配角色。

生产站点未登录时展示安全登录页，不会回退展示演示项目；本地仅在部署环境变量`SEED_DEMO_DATA=true`时使用管理员演示身份。

全新数据库只自动初始化标准节点模板与默认规则，不写入示例项目。若需要一次性演示数据，可在空的临时数据库中显式设置 `SEED_DEMO_DATA=true`；正式环境应保持为 `false`。已有项目库不会因该配置被覆盖或删除。演示项目按约七周一个标准里程碑的节奏铺排，正常情况下同一项目每个自然月最多出现一个标准里程碑；并行、自定义节点作为少量例外场景单独展示。
...
当前版本已完成高保真界面、Cloudflare D1/KV 持久化、独立账号身份权限、项目和用户管理、项目生命周期、负责人账号目录闭环、Excel批量导入、12节点模板治理、节点四态执行管理、项目当前阶段、多节点周报、自定义节点候选提升、规则版本、风险措施闭环、周报附件、站内及外部通知、催报升级、组合分析、延期概率预测、跨周期事后度量、跨项目资源容量与冲突治理、报表导出、核心工作流 API 及审计能力。后续将继续扩展预算与质量健康度、资源需求预测和更完整的业务分支回归。
