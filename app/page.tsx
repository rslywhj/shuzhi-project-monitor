    ...
    <section className="cockpit-main">
      <div className="heatmap-panel" ref={matrixRef}>
        <div className="heatmap-table" style={{ "--milestone-count": matrixMilestones.length } as React.CSSProperties}>
          <div className="heatmap-head"><div className="project-col project-sort-header"><button type="button" className={sort.field === "project" ? "active" : ""} aria-label={`按项目名称${sort.field === "project" && sort.direction === "asc" ? "倒序" : "正序"}排列`} aria-pressed={sort.field === "project"} onClick={() => toggleSort("project")}>项目 <i>{sort.field === "project" ? (sort.direction === "asc" ? "↑" : "↓") : "↕"}</i></button><button type="button" className={sort.field === "health" ? "active" : ""} aria-label={`按健康度评分${sort.field === "health" && sort.direction === "asc" ? "倒序" : "正序"}排列`} aria-pressed={sort.field === "health"} onClick={() => toggleSort("health")}>健康度 <i>{sort.field === "health" ? (sort.direction === "asc" ? "↑" : "↓") : "↕"}</i></button></div>{matrixMilestones.map((m, i) => <div key={`${m}-${i}`}><span>{String(i + 1).padStart(2, "0")}</span>{m}</div>)}</div>
          {filtered.map((p) => <div className="heatmap-row" key={p.id}>
            <button className="project-cell" onClick={() => onNavigate("project", p.id)}>
              <span><strong>{p.name}</strong><small>{projectPrimaryMilestone(p) ? `主：${projectPrimaryMilestone(p)?.name} · 并行${p.stageSummary?.parallelMilestoneIds.length ?? 0} · 遗留${p.stageSummary?.carryoverMilestoneIds.length ?? 0}` : `${p.owner} · ${p.org}`}</small></span><b>{p.score}</b>
            </button>
...
type OfficialProgressImportResult = {
  ...
  created?: number;
  updated?: number;
  baselineVersionFrom?: number;
  baselineVersionTo?: number;
  error?: string;
  summary: {
  ...
  return (
    <div className="modal-backdrop" onClick={busy ? undefined : onClose}>
      <section className="create-modal official-progress-import-modal" onClick={(event) => event.stopPropagation()} aria-busy={busy}>
        ...
        {result?.baselineConflicts.length ? <section className="import-validation-panel invalid baseline-conflict-panel"><div className="import-validation-head"><div><strong>将覆盖{result.baselineConflicts.length}项当前基线日期</strong><small>确认同步后立即采用Excel计划，并生成新的批准基线版本；原始V1和既有版本永久保留。</small></div><span>生成新版本</span></div><div className="import-error-table"><div><span>序号</span><span>节点</span><span>当前基线</span><span>Excel计划</span></div>{result.baselineConflicts.slice(0, 20).map((conflict) => <div key={conflict.sequence}><span>{conflict.sequence}</span><span>{conflict.name}</span><span>{conflict.current}</span><span>{conflict.workbook}</span></div>)}</div></section> : null}
        ...
        {phase === "success" && result ? <section className="import-success-state"><span>✓</span><h3>三级进度表同步完成</h3><p>已更新36个基线节点，新建{result.created ?? 0}个项目、更新{result.updated ?? 0}个项目。</p><dl><div><dt>导入批次</dt><dd>{result.importId}</dd></div>{result.baselineVersionTo && result.baselineVersionTo !== result.baselineVersionFrom ? <div><dt>基线版本</dt><dd>V{result.baselineVersionFrom} → V{result.baselineVersionTo}</dd></div> : null}</dl></section> : null}
        {error && <div className="form-error" role="alert">! {error}</div>}
